...
1. In the EMMs contrast analysis: both "label" v.s. "congruent-sound" contrast and "congruent-sound" v.s. "incongruent-sound" contrast should be significantly below zero.

2. In the Chi-square test: the model with the factor of interest (the congruency of sound cues) should fit the data significantoy better than the model without the factor of interest.
...
```
The results suggest that the full model (which includes the congruency factor) explain the data significantly better.
...
[TODO: (a) insights from follow-up exploratory analysis]
