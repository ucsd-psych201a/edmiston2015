...
### Planned Sample
We plan to have a sample size of n = 43 to be as close as possible to the original study. Participants must speak English to ensure comprehension in the task. 
...
The original study used a remote controller, while this study used computer for responses. We will use key responses in order to simulate buttons on controller, so as not to interfere with response times.  
